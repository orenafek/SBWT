...
class LyndonBijective$Test extends FunSuite {
  ...
  test("testRotations") {
    assertResult(List("ABC", "BCA", "CAB").map(_.toList))(GilScottBijectiveTransform.sort(GilScottBijectiveTransform.rotations(List("ABC".toList))))
  }
  ...
  test("inverse") {
    assertResult("now is the time for the truly nice people to come to the party")(GilScottBijectiveTransform.inverse("yoeyeeosreeeepi mhchlmhp tttnt puio wttcefn  ooati       rrotl".toList).mkString)
  }
